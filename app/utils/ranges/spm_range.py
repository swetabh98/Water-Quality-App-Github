# app/utils/ranges/spm_range.py
"""
SPM (Special Profile Mill) range row extractor.

Emits rows shaped for the central range checker:
    {"page": "spm_report.html", "system": <system>, "parameter": <param>, "value": <num>}

Matches inputs generated by spm_report.html:
  name="spm_<URLENCODED_SYSTEM>_<DB_KEY>_value"
where <DB_KEY> ∈ {
...
Notes:
- URL-decodes system names so they align with JSON (e.g., "ICW Mill").
- Canonicalizes parameter tokens to your JSON keys:
    Cond→conductivity, Turb→turbidity, M-Alk→malk, IRON→iron, Oil_and_Grease→oilgrease, etc.
...
def _coerce_float(token: Any) -> Optional[float]:
    """Best-effort numeric conversion; tolerates typos like '9.F5' -> 9.55."""
    if token is None:
        return None
...
_PARAM_ALIAS = {
    ...
    "zinc": "zinc",
    "phosphate": "phosphate",
    # "remark" intentionally not mapped (free text)
}

def _canon_param(label: str) -> Optional[str]:
    n = label.strip()
    # normalize variants from template/DB keys
    if n.lower() == "ph":
        key = "ph"
    elif n.lower() == "cond":
        key = "cond"
        ...
        # fallback: try a direct lowercase map
        key = n.lower()
...
RE_ROW = re.compile(rf"^spm_(?P<system>.+)_(?P<param>{PARAM_ALT})(?:_value)?$")
