...
rail_mill_reports_bp = Blueprint('rail_mill_reports', __name__)
...
@rail_mill_reports_bp.route('/rail_mill/fill_report/<int:dept_id>/<int:equip_id>', methods=['GET', 'POST'])
@login_required
def fill_report_rail_mill(dept_id, equip_id):
    ...
    if request.method == 'POST':
        ...
        db.session.flush()
        section = ReportSection(report_id=report.id, sheet_name="Rail Mill Report")
        ...
        # ✅ Range-alerts: run only after saving; emails only if any outliers
        handle_range_alerts(report)
        ...
        try:
            ...
            print(f"❌ Failed to send email for Rail Mill report {report.id}: {e}")

        flash("Rail Mill Report submitted successfully!", "success")
        return redirect(url_for('reports.summary'))

    return render_template("pages/rail_mill_report.html", department=dept, equipment=equip, sampling_time=datetime_now, existing_data={})
# --- Route to View a Rail Mill Report ---
@rail_mill_reports_bp.route('/rail_mill/report/<int:report_id>', endpoint='view_report_rail_mill')
@login_required
def view_report_rail_mill(report_id):
    """Displays a single Rail Mill report."""
    ...
    return render_template('pages/rail_mill_view.html', **context)

# --- Route to Edit a Rail Mill Report (Corrected) ---
@rail_mill_reports_bp.route('/rail_mill/report/<int:report_id>/edit', methods=['GET', 'POST'], endpoint='edit_report_rail_mill')
@login_required
def edit_report_rail_mill(report_id):
    """Handles editing an existing Rail Mill report."""
    report = Report.query.options(*_report_eager_options()).get_or_404(report_id)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            print(f"❌ Failed to send email for updated Rail Mill report {report.id}: {e}")

        flash(f"Report ID {report.id} updated successfully!", "success")
        return redirect(url_for('reports.summary'))

    # For a GET request, prepare the full context for the template
    ...
    return render_template('pages/rail_mill_edit.html', **context)

# --- Route to Delete a Rail Mill Report ---
@rail_mill_reports_bp.route('/rail_mill/report/<int:report_id>/delete', methods=['POST'], endpoint='delete_report_rail_mill')
@login_required
def delete_report_rail_mill(report_id):
    """Deletes a Rail Mill report."""
...
# --- Route to Download PDF for Rail Mill Report ---
from flask import send_file

@rail_mill_reports_bp.route('/rail_mill/report/<int:report_id>/pdf', endpoint='pdf_report_rail_mill')
@login_required
def pdf_report_rail_mill(report_id):
    context = _get_report_context(report_id)
    pdf = generate_pdf_report("pages/rail_mill_pdf.html", context)

    if not pdf:
        flash("Failed to generate PDF", "danger")
        return redirect(url_for('rail_mill_reports.view_report_rail_mill', report_id=report_id))

    filename = f"Rail_Mill_Report_{report_id}.pdf"
